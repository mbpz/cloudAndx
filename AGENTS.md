...
- 默认运行拓扑必须始终只有一个 `emulator` 运行容器和一个最终运行镜像。noVNC、WebSocket/VNC bridge、Android Emulator、ADB 代理、Device Bridge 和证据门禁必须在该容器内由同一 fail-closed 入口监督；不得为远程界面恢复 sidecar 或辅助运行容器。
...
- Compose 默认端口继续绑定 `127.0.0.1`。noVNC 的 HTTP/WebSocket 入口也必须绑定回环地址。需要跨主机访问时，只能通过受控的 HTTPS 反向代理、VPN 或零信任网关发布 noVNC，不得直接把内部控制端口绑定到 `0.0.0.0`。
