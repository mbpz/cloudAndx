...
远程 X11 桌面与 Android 的 `1080x2424` 显示尺寸一致，scrcpy 使用覆盖整个桌面的无边框窗口；noVNC 强制启用 `resize=scale` 并关闭 `view_only`，Android 画面会随浏览器可视区域自适应缩放，且不会被浏览器缓存的“仅查看”设置禁用输入。浏览器链路显式使用 scrcpy SDK 鼠标与键盘注入，左键点击和按住拖动按 Android 触控语义处理；视频限制为 30 FPS、零缓冲并使用 4 Mbps 码率，优先降低操作延迟。
