...
Port `6080` serves the encrypted HTTPS/WSS browser interaction path from the same runtime container. A pinned scrcpy 4.1 client controls the internal ADB serial and renders into Xvfb; x11vnc exposes that display only on container loopback, and pinned websockify/noVNC translates it for the browser. scrcpy uses SDL's core X11 input path because x11vnc's synthetic button and drag events are not delivered reliably through SDL XInput2; SDK mouse mode then maps those events to Android touchscreen DOWN/MOVE/UP semantics. Compose publishes only noVNC on host loopback. A persistent self-signed certificate is created on first start, so the browser must trust it once. Raw VNC port `5900`, the X11 socket, ADB internals and emulator gRPC are never browser-facing. Closing or reconnecting the browser does not create another Android instance.
